...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('ElevenLabs TTS Streaming request received');
    ...
    if (!response.ok) {
      ...
      console.error('ElevenLabs TTS Streaming API error:', errorText);
      return {
        ...
        body: JSON.stringify({
          error: `ElevenLabs TTS Streaming failed: ${response.status}`,
          details: errorText
        })
      };
    }

    // Get the streaming audio data
    const audioBuffer = await response.arrayBuffer();
    const audioBase64 = Buffer.from(audioBuffer).toString('base64');

    console.log('TTS Streaming conversion successful');
    ...
    console.error('ElevenLabs TTS Streaming function error:', error);
